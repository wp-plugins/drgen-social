<?php
/*
Plugin Name: DrGEN Social
Plugin URI: http://www.drgen.com.ar/ciencialinks/static/wp-plugin/
Description: Genera botones para el envío o la votación en distintas redes sociales: Facebook, Twitter, #CienciaLinks, Google Buzz, Delicious, Meneame y Bitacoras.com. Opcionalmente puede mostrar contadores con el número de votos o veces que se ha compartido (según la red).
Author: DrGEN
Version: 1.0
Author URI: http://www.drgen.com.ar/
*/
...
class ic_BeSocial extends ic_Plugin {
	...
	var $version	= '1.0';
	var $buttons	= array('meneame', 'bitacoras', 'ciencialinks', 'delicious', 'buzz', 'facebook', 'twitter');
	var $objects	= array();
	...
	function init() {
		...
		$this->addOption('position', array('label' => __('Position', 'besocial'), 'type' => 'select', 'value' => 'bottom', 'enum' => array('bottom' => __('Bottom', 'besocial'), 'top' => __('Top', 'besocial'), 'none' => __('None', 'besocial')), 'info' => __('If "None" is selected you must use <code>drgen_social_Buttons()</code> in <code>single.php</code>', 'besocial')));
	 ...
	 */
	function initAdmin() {
		add_options_page('DrGEN Social', 'DrGEN Social', 'manage_options', $this->getPath(basename(__FILE__)), array(&$this, 'getForm'));
	}
	...
	function getForm() {
		echo "<div class='wrap'>\n";
		printf("<h2>%s DrGEN Social V%s</h2>\n", __('Configure', 'DrGEN Social'), $this->version);

		if ( isset($_POST['submit']) && $this->setForm() ) {
			printf("<div id='message' class='updated fade'><p><strong>%s</strong></p></div>\n", __('Options saved', 'DrGEN Social'));
		}
	...
	function getScripts( $vars ) {
		...
		if ( $this->getOption('counters') ) {
			wp_enqueue_script('besocial', $this->getURI('drgen-social.js'), array(), $this->version, true);
...
class drgen_social_Button extends ic_Plugin {
	...
	function drgen_social_Button( $count ) {
		$this->count = $count;
		$this->init();
	}
...
class ic_BeSocial_Twitter extends drgen_social_Button {
...
class ic_BeSocial_Facebook extends drgen_social_Button {
	...
	function initButton( $post ) {
		...
		$this->setTitle(__('Compartir el post en', 'besocial') . ' Facebook');
		$this->setHref('http://www.facebook.com/sharer.php?u=' . rawurlencode($post['url'])  . '&amp;t=' . rawurlencode($post['title']) . '&amp;src=sp');
	}
}

class ic_BeSocial_Bitacoras extends drgen_social_Button {
	...
	function initButton( $post ) {
		...
		$this->setTitle(__('Enviar el post a', 'besocial') . ' Bitacoras.com');
		$this->setHref('http://bitacoras.com/anotaciones/' . preg_replace('/^https?:\/\//', '', $post['url']));
	}
}

class ic_BeSocial_Meneame extends drgen_social_Button {
	...
	function initButton( $post ) {
		...
		$this->setTitle(__('Enviar el post a', 'besocial') . ' Meneame');
		$this->setHref('http://www.meneame.net/submit.php?url=' . $post['url'] . '&amp;title=' . rawurlencode($post['title']));
	}
}

class ic_BeSocial_Delicious extends drgen_social_Button {
	...
	function initButton( $post ) {
		...
		$this->setTitle(__('Guardarlo en', 'besocial') . ' Delicious');
		$this->setHref('http://www.delicious.com/save?v=5&amp;noui&amp;url=' . $post['url'] . '&amp;title=' . rawurlencode($post['title']));
	}
}

class ic_BeSocial_ciencialinks extends drgen_social_Button {
	...
	function initButton( $post ) {
		$this->setText('ciencialinks');
		$this->setTitle(__('Enviar el post a', 'besocial') . ' ciencialinks');
		$this->setHref('http://www.drgen.com.ar/ciencialinks/submit.php?url=' . $post['url']);
	}
}

class ic_BeSocial_Buzz extends drgen_social_Button {
